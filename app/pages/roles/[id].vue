...
const { permissions } = storeToRefs(permissionStore) as { permissions: Ref<Permission[]> };

const submitting = ref(false);

const showSaveToast = (success: boolean, message: string) => {
  const t = useToast();
  t.add({ severity: success ? 'success' : 'error', summary: success ? 'Success' : 'Error', detail: message, life: 3000 });
};
...
const form = ref<Role>({ id: id as unknown as number, name: '' });
...
const save = async (e: FormSubmitEvent) => {
  ...
  const payload = { name: values.name, permissions: selectedPermissions.value } as { name: string; permissions: Array<number | string> };
  try {
    // convert permission ids to strings if back-end expects strings
    const permIds = selectedPermissions.value.map((v) => String(v));
    await roleStore.editRole(Number(id), { ...payload, permissions: permIds } as unknown as Partial<Role & { permissions?: string[] }>);
...
<template>
  <div class="bg-white p-4">
    <Breadcrumb :home="{ icon: 'pi pi-home' }" :model="[{ label: 'Roles' }, { label: form.name || 'Role' }]" />
        ...
        <Form
          ...
          <div class="flex flex-col gap-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Panel v-for="(group, prefix) in groupedPermissions" :key="prefix" :header="prefix.toUpperCase()" class="flex flex-col gap-1 ">
                    <label>Permissions :: {{  selectedPermissionsPerGroup[prefix]?.length ?? 0 }}</label>

                    <Listbox v-model="selectedPermissionsPerGroup[prefix]" filter :options="group" multiple option-label="label" option-value="label" class="mt-4"   />
                </Panel>
