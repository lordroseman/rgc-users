...
const fetchRoles = roleStore.fetchRoles as () => Promise<void>;
...
const toggle = (event: MouseEvent, data: Role) => {
  selected.value = data;
...
const loadRoles = useDebounceFn(async () => {
  loading.value = true;
  await fetchRoles();
  loading.value = false;
}, 300);

defineExpose({ loadRoles });
</script>
  ...
  <div class="card">
    <DataTable
      ...
      selection-mode="single"
        @row-click="(e) => navigateTo(`/roles/${e.data.id}`)"
    >
      <template #header>
        <div class="flex justify-between gap-4">
          ...
          <div class="flex-shrink-0 flex gap-4">
            ...
            <Button icon="pi pi-plus" label="Add Role" class="flex-shrink-0" @click="openCreate" />
          </div>
        </div>
      </template>
      ...
      <template #empty>
        <div class="flex flex-col items-center justify-center h-64">
          ...
          <span v-if="loading">Loading roles...</span>
